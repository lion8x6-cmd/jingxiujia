...
def TBL(headers,rows):
    ...
    for i,h in enumerate(headers):
        ...
        r.font.color.rgb=RGBColor(0xFF,0xFF,0xFF); r.font.name='微软雅黑'
    ...
    for ri,row in enumerate(rows):
        for ci,val in enumerate(row):
            ...
            r=c.paragraphs[0].add_run(str(val)); r.font.size=Pt(9.5); r.font.name='微软雅黑'
            r.element.rPr.rFonts.set(qn('w:eastAsia'),'微软雅黑')
...
def B(t):
    ...
    r=pp.add_run(t); r.font.size=Pt(10.5); r.font.name='微软雅黑'
    r.element.rPr.rFonts.set(qn('w:eastAsia'),'微软雅黑')
