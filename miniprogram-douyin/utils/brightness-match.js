...
function dataURLToFile(dataUrl) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const filePath = platform.env.USER_DATA_PATH + '/bm_' + Date.now() + '_' + Math.random().toString(36).substr(2, 6) + '.png';
      fs.writeFile({
        filePath,
        data: base64,
        encoding: 'base64',
...
function matchBrightness(resultPath, refPath, options) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      const canvas = platform.createOffscreenCanvas({ type: '2d', width: SAMPLE_SIZE, height: SAMPLE_SIZE });
