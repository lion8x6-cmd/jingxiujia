...
Page({
  ...
  onLoad(options) {
    ...
    if (!skill) {
      platform.showToast({ title: '未知 Skill', icon: 'none' });
      setTimeout(() => platform.navigateBack(), 800);
      return;
    }

    // 设置导航标题
    platform.setNavigationBarTitle({ title: skill.name });
  ...
  // 选择图片（抖音端使用 tt.chooseImage，参数兼容更稳定）
  onChooseImage() {
    platform.chooseImage({
      ...
      fail: (err) => {
        console.warn('[lab] chooseImage:', err);
      }
    });
  },
  ...
  onSliderTap(e) {
    ...
    // 简化：点击位置占比
    const step = Math.round((touchX / (platform.getSystemInfoSync().windowWidth - 128)) * (count - 1));
  ...
  onCopyPrompt() {
    ...
    platform.setClipboardData({
      ...
      success: () => platform.showToast({ title: '已复制提示词', icon: 'success' })
    });
  },
  ...
  async onGenerate() {
    ...
    if (!prompt || !prompt.trim()) {
      platform.showToast({ title: '提示词为空', icon: 'none' });
      return;
    }
    ...
    try {
      ...
      platform.showToast({ title: err.message || '生成失败', icon: 'none' });
    }
  },
  ...
  async onSaveResult() {
    ...
    try {
      ...
      if (isAuthDenied(err)) {
        ...
        platform.showToast({ title: '保存失败', icon: 'none' });
      }
    }
  },
