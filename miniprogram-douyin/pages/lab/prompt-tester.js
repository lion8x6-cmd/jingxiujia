...
Page({
  ...
  goHistory() {
    platform.navigateTo({ url: '/pages/lab/prompt-history' });
  },
  ...
  savePrompt() {
    ...
    if (!editName.trim() || !editPrompt.trim()) {
      platform.showToast({ title: '名称和提示词不能为空', icon: 'none' });
      return;
    }
    if (editingId) {
      ...
      platform.showToast({ title: '已更新', icon: 'success' });
    } else {
      promptLib.addPrompt({
        name: editName,
        category: editCategory,
  ...
  previewImage() {
    if (this.data.imagePath) {
      platform.previewImage({ urls: [this.data.imagePath] });
    }
  },

  previewResult() {
    if (this.data.resultUrl) {
      platform.previewImage({ urls: [this.data.resultUrl] });
    }
  },
  ...
  async generate() {
    ...
    try {
      ...
      platform.pageScrollTo({ selector: '.result-section', duration: 300 });
    } catch (err) {
      this.stopProgressAnim();
  ...
  useAsInput() {
    ...
    platform.showToast({ title: '已作为新参考图', icon: 'none' });
  },
