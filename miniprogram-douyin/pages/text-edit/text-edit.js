...
Page({
  ...
  onImageReady() {
    const query = platform.createSelectorQuery();
  ...
  async submitEdit() {
    if (!this.data.newText.trim()) {
      platform.showToast({ title: '请输入替换文字', icon: 'none' });
      return;
    }
    ...
    if (hasUnsavedChanges) {
      ...
      try {
        ...
        setTimeout(() => {
          ...
          platform.showToast({ title: '处理完成', icon: 'success' });
          setTimeout(() => platform.navigateBack(), 1500);
        }, 2000);
      } catch (err) {
        this.setData({ processing: false });
        platform.showToast({ title: '处理失败', icon: 'none' });
      }
    }
  },
