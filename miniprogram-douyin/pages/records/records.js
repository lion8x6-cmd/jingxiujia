...
Page({
  ...
  onPullDownRefresh() {
    ...
    platform.showToast({ title: '已刷新', icon: 'none' });
  },
  ...
  deleteSelected() {
    ...
    if (!ids.length) {
      platform.showToast({ title: '请先选择记录', icon: 'none' });
      return;
    }
    ...
    platform.showModal({
      ...
      success: (res) => {
        ...
        platform.showToast({ title: `已删除 ${removed} 条`, icon: 'none' });
        this.setData({
          selectMode: false,
          selectedIds: [],
          selectedIdsMap: {},
  ...
  saveAllToAlbum() {
    ...
    if (!completedRecords.length) {
      platform.showToast({ title: '暂无可转存的图片', icon: 'none' });
      return;
    }
        ...
        success: (res) => {
          if (res.confirm) {
            ...
            if (result.ok) {
              ...
              platform.showToast({ title: '暂无可重试的任务', icon: 'none' });
            }
          }
        }
      });
